...
void LogManager::setOutLevel(int nLevel)
{
	m_nLogOutputLevel = nLevel;
}
...
void LogManager::setCurrentPathFileName(char *cLogPathFileName, const char *cCurrentData,int nFileNum)
{
	...
	if (nFileNum != 0)
	{
		...
		string strLogFileNum = Converter::toString(nFileNum);
...
{
	if (logLevel > m_nLogOutputLevel) //4 3 2 1 0
	{
		return;
	}
	char Buff[1024];
